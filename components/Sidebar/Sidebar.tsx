...
export const Sidebar: FC<Props> = ({ loading, conversations, lightMode, selectedConversation, apiKey, folders, onCreateFolder, onDeleteFolder, onUpdateFolder, onNewConversation, onToggleLightMode, onSelectConversation, onDeleteConversation, onToggleSidebar, onUpdateConversation, onApiKeyChange, onClearConversations, onExportConversations, onImportConversations }) => {
  ...
  return (
    <aside className={`h-full flex flex-none space-y-2 p-2 flex-col bg-[#283618] w-[260px] z-10 sm:relative sm:top-0 absolute top-12 bottom-0`} style={{color: "#DDA15E"}}>
      <header className="flex items-center">
        <button
          className="flex gap-3 p-3 items-center w-[190px] rounded-md hover:bg-emerald-500/10 transition-colors duration-200 text-white cursor-pointer text-sm flex-shrink-0 border border-white/20"
          onClick={() => {
            onNewConversation();
            setSearchTerm("");
          }}
        ...
        <button
          ...
          onClick={() => onCreateFolder("Nueva Carpeta")}
        >
